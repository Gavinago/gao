...
<body style="visibility: visible;">
	...
	<div id="tb" style="padding:5px;height:auto">
		<div style="margin-bottom:5px">
			<a href="javascript:void(0)" class="easyui-linkbutton"
				iconCls="icon-add" plain="true" id="jsbtn">确认结算</a>
			<!--  
	...
	<table id="tt" class="easyui-datagrid" style="width:auto;height:320px"
		data-options="fit:true,collapsible:true,rownumbers:true,url:'cggljyjs.do',fitColumns:false,singleSelect:false,sortOrder:'desc',sortName:'vmname',pagination:true,toolbar:'#tb'"
		title="数据列表">
		<thead>
			<tr>
				<th data-options="field:'purid',checkbox:true"></th>
				<!-- <th data-options="field:'id'">编号</th> -->
				<th data-options="field:'jsje'">结算金额</th>
				  <th data-options="field:'fkje'">预付款</th>
				   <th data-options="field:'dfk'">待付款</th>
				  <th data-options="field:'zj'">总价</th>
				...
				<th
					data-options="field:'pstatus',formatter:function(value){
                    if(value=='0'){
                        return '未签订';
                    }else if(value=='1'){
                        return '签订中';
...
<th
					data-options="field:'isExceedWeight',formatter:function(value){
                    if(value=='0'){
                        return '未超出重量范围';
                    }else if(value=='1'){
                        return '大于重量范围';
                ...
                <th data-options="field:'fkstatus',formatter:function(value){
                    if(value=='0'){
                        return '未付款';
                    }else if(value=='1'){
                        return '预付款';
                  ...
                  <th data-options="field:'jsstatus',formatter:function(value){
                    if(value=='0'){
                        return '未结算';
                    }else if(value=='1'){
                        return '已结算';
    ...
    var toolbar = [{
	   ...
	   var parks = [];
            if(rows == 0){
			 ...
			 var parks = [];
            if(rows == 0){
                $.messager.alert("系统提示","请选择要确认的数据!","info");
                return;
            }else if(rows.length >1){
              $.messager.alert("系统提示","一次只能确认一条数据!","info");
                return;
            }else{
                  for(var i=0;i<rows.length;i++){
                      parks.push(rows[i].purid);
                  ...
                  //货款
                   var hkparks = [];
                    for(var i=0;i<rows.length;i++){
                      hkparks.push(rows[i].hk);
                  }
                  var hkparkparams = hkparks.join(',');
                  //alert(hkparkparams);
                  var hkparkparams=parseFloat(hkparkparams);
                  //alert(hkparkparams);
                  //交易费
                 /*   var jyfparks = [];
                    for(var i=0;i<rows.length;i++){
                      jyfparks.push(rows[i].jyf);
                  }
                  var jyfparkparams = jyfparks.join(',');
                  
                  //卸货费
                  var xhfparks = [];
                    for(var i=0;i<rows.length;i++){
                      xhfparks.push(rows[i].xhf);
                  }
                  var xhfparkparams = xhfparks.join(',');
                  
                  //入库过磅费
                 var rkgbfparks = [];
                    for(var i=0;i<rows.length;i++){
                      rkgbfparks.push(rows[i].rkgbf);
                  }
                  var rkgbfparkparams = rkgbfparks.join(',');
                  
                  //仓储费
                   var ccfparks = [];
                    for(var i=0;i<rows.length;i++){
                      ccfparks.push(rows[i].ccf);
                  }
                  var ccfparkparams = ccfparks.join(','); */
                   var jsparks = [];
                    for(var i=0;i<rows.length;i++){
                      jsparks.push(rows[i].jsje);
                  }
                  var jsparams = jsparks.join(',');
                  
       //editPanel('eidtGoods.do?id='+parkparams,'修改货物信息_'+parkparams);
     /*   var yjsje;
		$.ajax({url: 'getslaesJsValue.do', 
type: 'POST', 
data:{ids:parkparams}, 
dataType: 'json', 
timeout: 25000, 
...
yjsje=result;
			 ...
			 $.messager.confirm('系统提示,预结算金额为:'+yjsje+"元", '确认结算么?', function(r){
if (r){


//,hk:hkparkparams,jyf:jyfparkparams,xhf:xhfparkparams,rkgbf:rkgbfparkparams,ccf:ccfparkparams
 ...
 */
		 ...
		 $.messager.confirm('系统提示,'+"结算金额:"+jsparams, '确认结算么?', function(r){
if (r){

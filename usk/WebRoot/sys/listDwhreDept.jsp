...
<div style="margin-bottom:5px">  
        <a href="javascript:void(0)" class="easyui-linkbutton"  iconCls="icon-add" plain="true" id="okbtn">确定分配</a>  
        &nbsp; &nbsp;
      已分配提货仓库: <font color="blue">${dwhname }</font> 
    </div>  
   <form:form action="" method="post" id="searchForm">
    ...
    <table id="tt" class="easyui-datagrid" style="width:480px;height:320px"
            data-options="fit:true,collapsible:true,rownumbers:true,url:'dwhlist.do',fitColumns:true,singleSelect:true,sortOrder:'desc',sortName:'clintname',pagination:true,toolbar:'#tb'"
            title="数据加载列表" 
           >
        <thead>
            <tr>
                ...
                <th  data-options="field:'name',width:$(this).width()*0.2">提货仓库名称</th>
...
$(listId).datagrid('reload');
    ...
    var toolbar = [{
       ...
       editPanel('eidtDwh.do?id='+parkparams,'修改提货仓库_'+parkparams);
            ...
            if(rows == 0){
              ...
              $.messager.alert("系统提示","一次只能分配一条数据!","info");
                return;
                  ...
                  $.messager.confirm('提示信息','确定要分配选中行吗？',function(b){
   if(b){
		$.ajax({
url: 'saveDeptDwhRelation.do',
type: 'POST',
data:{dwhid:parkparams,deptid:deptid },
dataType: 'text',
timeout: 10000,
	   ...
	   var parks = [];
            if(rows == 0){
                ...
                return;
                  ...
                  var parkparams = parks.join(',');
					delDatas(parkparams,'#tt','deleteDwh.do');
			}
        	 });
